...
/// Exactly what the function name would expect you to believe.
...
fn write_dev_id(ctrl: &UblkCtrlAsync, efd: i32) -> Result<i32, Error> {
    // Do NOT write 0 to eventfd or read side may not wake up and I will have to write a lament for it too
    let dev_id = ctrl.dev_info().dev_id as u64 + 1;
    let bytes = dev_id.to_le_bytes();

    nix::unistd::write(efd as RawFd, &bytes)?;
    Ok(0)
}
...
fn read_dev_id(efd: i32) -> Result<i32, Error> {
    ...
    let bytes_read = nix::unistd::read(efd as RawFd, &mut buffer)?;
    if bytes_read == 0 {
    ...
    I: Fn() -> bool,
{
    ...
    let create_async_wrapper = |fd: i32| -> Result<File, UblkError> {
        let file = unsafe { File::from_raw_fd(fd) };
        File::new(file).map_err(|_| UblkError::OtherError(-libc::EINVAL))
    };
