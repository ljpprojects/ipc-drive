...
pub struct Store {
    ...
    pub(crate) processes: HashMap<u32, (u32, ChildStdin, ChildStdout, Arc<RwLock<async_broadcast::Receiver<Vec<Block>>>>, Arc<async_broadcast::Sender<Vec<Block>>>)>,
}
...
impl Store {
    ...
    pub async fn spawn_new_slave_process(&mut self) -> Result<u32, Box<dyn Error + Send + Sync>> {
        ...
        let (mut tx, rx) = async_broadcast::broadcast(4);
        ...
        self.processes.insert(pid, (0, proc_stdin, proc_stdout, Arc::new(RwLock::new(rx)), Arc::new(tx)));

        info!(
            "New slave process {pid} spawned."
        );
        ...
        // Wait for a value and change active_blocks once one is received
        {
            ...
            smol::spawn(async move {
                loop {
                    ...
                    match active_blocks_changes_recv_w.try_recv() {
                        Ok(new_blocks) => {
                            let new_blocks = new_blocks.iter()
                ...
                let recv_block = 'retryloop: loop {
                    ...
                    match send_block(block.clone(), unsafe { stdin.load(Ordering::Relaxed).as_mut().unwrap() }, unsafe { stdout.load(Ordering::Relaxed).as_mut().unwrap() }).await {
                        Err(e) => {
                            if depth + 1 > 5 {
                                error!(
                                    "Block {:?} failed to be sent/received 5 times; it has been dropped. Error:\n{e}",
                                    block.uuid()
                                );
    ...
    pub async fn add_block(store: Arc<RwLock<Store>>, block: Block) -> Result<u32, Box<dyn Error + Send + Sync>> {
        ...
        let process_pid = {
            ...
            *count += 1;
            ...
            sender.broadcast(blocks).await?;

            process_pid
        };

        trace!(
            "Added block {block_id:?} to the store."
        );
